 ...
 * /auth/profile/:
 *  get:
 *    description: Get profile of the current logon user
 *    tags: ["auth"]
 *    responses:
 *      200:
 *        description: user profile successfuly fetched
 *        content:
 *          application/json:
 *            schema:
 *              $ref: '#/components/schemas/UserSchema'
 ...
 *              password: test_test
 *      401:
 *        description: user not yet logged in
 *        content:
 *          application/json:
 *            example:
 *              status: 401
 *              logged: false
 *              message: You are not authorized to app.
 */
router.get('/profile', AuthComponent.profile);
